...
### 3.12 K · 政府关系档案(2026-04-24 新增模块)

> **2026-04-24 决策融入** —— T4 对比矩阵 §5 借鉴 AI 版的"GovContact 关键人档案 + GovOrg 政府机构档案",用户拍板**采纳变体**(配套形式 / 不做字段级保护 / Visit 双轨关联 / 不做分层 / 评分 F 类 / 不做离职跟踪 / 进 V0.5 试点)。本节是数据模型层面的扩展,不改产品定位与设计哲学(沿用 7.1.1 ExportRecord 水印路线,**不引入字段级权限**)。

| 代号 | 功能 | 优先级 | 说明 | 来源 |
|---|---|---|---|---|
| **K1** | 政府机构(GovOrg)CRUD | **P0** | 字段:名称 / 简称 / 区划 / 层级 / 上级机构 / 职能标签 / 地址(详见 4.3.6) | T4 §5 借鉴 |
| **K2** | 关键人(GovContact)CRUD | **P0** | 字段:姓名 / 所在机构 / 职务 / 电话 / 微信 / 邮箱 / 备注;**不做分层 tier**(2026-04-24 用户拍板) | T4 §5 借鉴 |
| **K3** | Visit 关联到 GovContact(双轨) | **P0** | `Visit.contact_id` 优先选档案;`contact_person` 字符串作为兜底(没建档时直接填,后续可补建关联) | 用户 2026-04-24 拍板 |
| **K4** | 机构 ↔ 关键人 1-N 关系 | **P0** | 1 个机构挂多个关键人;关键人挂 1 个机构;机构含 `parent_org_id` 上级链 | K1 + K2 配套 |
| **K5** | 机构 / 关键人列表 + 详情页 | **P0** | 列表筛选(按区划 / 按职能);详情页含历史 Visit 列表 | 6.3 页面地图 |
| **K6** | 关键人活跃度评分 | **F 类(F13)** | 系统自动算(基于 Visit 频次 + `last_engaged_at`);需要 V1.0 后档案积累足够数据 | 用户 2026-04-24 → F13 |

**MVP 对应**:K1-K5 全 P0;K6 进 F 类(F13,V1.2+)

**说明**

- **不做的**(2026-04-24 用户拍板):
  - 关键人分层 tier(核心 / 重要 / 普通)
  - 字段级敏感保护(沿用 7.1.1 ExportRecord 水印)
  - 关键人离职 / 调岗主动跟踪
  - 关系活跃度评分 MVP(降为 F13)
- **数据归属转移(G23)联动**:owner 离职时,owner 创建的 GovOrg / GovContact 档案随之转移,沿用现有 G23 流程
- **权限模型**:与现有"自条目可编 + 全读基线"一致 —— 任何 `local_ga` / `central_ga` 都可读所有档案,只能编辑自己创建的;详见 5.1.5
- **D 全部不开**:档案删除会导致历史 Visit 关联孤立;离职 / 调岗只能改 `status: inactive`(`active` ⇄ `inactive` 由 owner 手动)
...
| K 政府关系档案 | K1 GovOrg CRUD / K2 GovContact CRUD / K3 Visit 关联(双轨)/ K4 机构↔关键人 / K5 列表+详情(2026-04-24 新增) |

**P0 总量**:约 **53** 功能点(2026-04-24 K 模块融入,从 48 → 53;增量 K1-K5)
...
- **4.3.6(政府端档案)是 2026-04-24 关键人档案融入时新增**:语义上属"属地业务延伸 —— 拜访的对象方";放在 4.3 末尾(而非新开 4.4)是为了**保留 4.4-4.8 原节号便于跨章引用稳定**(第 5 章 5.1.x、第 7 章 7.1.x、T4 §5 等都已固化引用)
...
| `department` | string | ✅ | 拜访对象部门(K3 启用后,若 contact_id 已选,自动从 GovContact.gov_org_id 关联机构带出;否则手填) | `杭州市发改委` |
| `contact_id` | UUID | ⬜ | 关联 GovContact 档案(**K3 双轨**,2026-04-24 新增,优先) | |
| `contact_person` | string | ✅ | 对接人(`contact_id` 为空时使用,作为兜底) | `李主任` |
| `contact_title` | string | ⬜ | 对接人职务(同上,`contact_id` 为空时使用) | `副主任` |
...
- **K3 双轨**(2026-04-24 新增):`contact_id` 优先(已建档时,通过下拉搜索 GovContact 选择);为空时使用 `contact_person` + `contact_title` 字符串字段(没建档时直接填,后续可在 Visit 详情或关键人列表补建关联)
- **`contact_id` 写入权限**:同 Visit 主体,即 `CUself`(拜访人自己写)
...
#### 4.3.6 GovOrg + GovContact(政府端档案,2026-04-24 新增)

> **K1 / K2 实体** —— T4 对比矩阵 §5 借鉴 AI 版的"GovContact 关键人档案 + GovOrg 政府机构档案",2026-04-24 用户拍板融入(**采纳变体**:配套形式 / 不做字段级保护 / 不做分层 / 不做评分 MVP / 不做离职跟踪)。放在 4.3 末尾是因为档案的核心交互场景是"被 Visit 关联"(属地业务延伸的"对象方"),保留 4.4-4.8 原节号便于跨章引用稳定。
...
|---|---|---|---|---|
| `id` | UUID | ✅ | | `org_xyz` |
| `name` | string(100) | ✅ | 机构全称 | `杭州市发展和改革委员会` |
| `short_name` | string(50) | ⬜ | 简称 | `杭州发改委` |
| `region_code` | string(6) | ✅ | 所在行政区划 | `330100` |
| `level` | enum | ✅ | `national` / `provincial` / `municipal` / `district` | `municipal` |
| `parent_org_id` | UUID | ⬜ | 上级机构(自引用,可建上下级关系链) | |
| `function_tags` | string[] | ⬜ | 职能标签 | `["能源","发改"]` |
| `address` | string | ⬜ | 办公地址 | |
| `status` | enum | ✅ | `active` / `inactive`(撤销 / 合并时手动改) | `active` |

**说明**
- 上级机构链(`parent_org_id`)支持递归查询(如"国家发改委 → 省发改委 → 市发改委")
- 不做"机构合并 / 拆分"的复杂迁移逻辑(MVP);发生时 owner 手动改 `status` 并新建即可
...
|---|---|---|---|---|
| `id` | UUID | ✅ | | `cnt_xyz` |
| `name` | string(50) | ✅ | 姓名 | `李主任` |
| `gov_org_id` | UUID | ✅ | 所在机构(关联 GovOrg) | `org_xyz` |
| `title` | string(50) | ⬜ | 职务 | `副主任` |
| `phone` | string(20) | ⬜ | 联系电话 | |
| `wechat` | string(50) | ⬜ | 微信号 | |
| `email` | string(100) | ⬜ | 邮箱 | |
| `notes` | text | ⬜ | 备注(包括偏好、历史等) | |
| `status` | enum | ✅ | `active` / `inactive`(离职 / 调岗后由 owner 手动改) | `active` |
| `last_engaged_at` | datetime | ⬜ | 最近一次拜访时间(由 K6 / F13 系统计算,MVP 阶段为空) | |

**关键设计决策(2026-04-24 用户拍板)**

| 项 | 决策 | 理由 |
|---|---|---|
| **字段级敏感保护** | **不做** | 沿用 7.1.1 ExportRecord 水印路线;关键人档案在系统里能看到的角色本身就是受信范围,字段级反而麻烦;若未来真出现"私人维护需求",再升级 |
| **关键人分层 tier**(核心 / 重要 / 普通) | **不做** | MVP 仅"档案存在 / 不存在"二态认知,不做主观分级 |
| **关键人离职 / 调岗主动跟踪** | **不做** | MVP 仅 `status` 字段供 owner 手动改;F 类候选未来评估 |
| **关系活跃度评分**(系统自动算) | **F 类(F13,V1.2+)** | 需要 V1.0 后档案积累足够数据 + 评分公式定义 |

##### 编辑权限速览(详细见第 5 章 5.1.5)

- `local_ga`:可创建 GovOrg / GovContact;可编辑自己创建的(沿用 1.3 B11 "他人条目不可编")
- `central_ga`:可创建 GovOrg / GovContact(中台对接的部委联系人);可编辑自己创建的
- `lead` / `pmo`:可读全部,不可编辑
- `sys_admin`:数据校正(写 `sensitive_data_edit` AuditLog)
- **D 全部不开**:档案删除会导致历史 Visit 关联孤立;离职 / 调岗只能改 `status: inactive`

##### Visit ↔ GovContact 关联(K3 双轨,详细见 4.3.4)

- Visit 录入时,`contact_id` 是首选(下拉搜索 GovContact)
- 没建档的对接人:仍用 `contact_person` + `contact_title` 字符串字段(原有逻辑保留)
- 后续可补建档:打开 Visit 详情 → 点"建档"按钮 → 自动用字符串字段预填表单 → 保存 GovContact 后回写 `contact_id`
- 1 个 GovContact 关联多个 Visit(N-1)
- GovContact 详情页的"历史 Visit 列表"由查询 `Visit.contact_id = current_id` 得到

##### 数据归属转移(G23)联动

- owner 离职时,owner 创建的 GovOrg / GovContact 档案随之转移,沿用现有 G23 流程(见 4.x 实体的 `created_by`)
- 不做单独的"档案转移"特殊路径
...
erDiagram
    ...
    GovOrg }o--|| Region : "located_at"
    GovOrg }o--o| GovOrg : "parent_of"
    GovOrg ||--o{ GovContact : "employs"
    GovContact ||--o{ Visit : "engaged_in"
    User ||--o{ GovOrg : "created_by"
    User ||--o{ GovContact : "created_by"
...
| K1 / K2 政府端档案融入 | 4.3.6 GovOrg + GovContact + 4.3.4 Visit.contact_id 双轨(K3)+ 4.8 ER 图(2026-04-24 新增) |
...
#### 5.1.5 政府端实体(2026-04-24 新增)

| 实体 | local_ga | pmo | lead | central_ga | sys_admin |
|---|---|---|---|---|---|
| **GovOrg**(政府机构) | **C + Uself** | R | R | **C + Uself** | **U✦**(校正) |
| **GovContact**(关键人) | **C + Uself** | R | R | **C + Uself** | **U✦**(校正) |

**说明**

- `local_ga` / `central_ga` 都可创建 GovOrg / GovContact(场景:属地走访新机构 + 中台对接的部委联系人);MVP **不做角色专属**,谁先建谁是 owner
- `Uself`:他人创建的档案,只能读不能改(对齐 1.3 B11)
- `pmo` / `lead`:可读全部,不可编辑(对应 1.2 / 1.1 的"看 + 决策"定位)
- `sys_admin`:数据校正,写 `sensitive_data_edit` AuditLog
- **D 全部不开**:档案删除会导致历史 Visit 关联孤立;离职 / 调岗只能改 `status: inactive`(`active` ⇄ `inactive` 算状态变更,但不开复杂状态机,直接 owner 手动改)
- **不做字段级权限**(2026-04-24 用户拍板,沿用 7.1.1 ExportRecord 水印路线):phone / wechat / notes 等字段对所有可读档案的角色一视同仁
- **Visit.contact_id 写入**(K3 双轨):同 Visit 主体 `CUself`(visitor 自己写,见 5.1.2 + 4.3.4)
...
├─ 关系档案(全角色 R / local_ga + central_ga 可 CUself,K 模块,2026-04-24 新增)
│  ├─ 机构列表(K1 / K5)
│  └─ 关键人列表(K2 / K5)
...
| 机构列表 | `/console/orgs` | 全角色 R / local_ga + central_ga 可 CUself | K1 / K5 | 单条详情 → 历史 Visit + 该机构关键人列表 |
| 机构详情 | `/console/orgs/:id` | 全角色 R / owner 可 U | K1 / K5 | 编辑(自己创建的)/ 关键人列表 / 上下级机构链 |
| 关键人列表 | `/console/contacts` | 全角色 R / local_ga + central_ga 可 CUself | K2 / K5 | 单条详情 → 历史 Visit + 所属机构 |
| 关键人详情 | `/console/contacts/:id` | 全角色 R / owner 可 U | K2 / K5 | 编辑(自己创建的)/ 历史 Visit 列表 / 所属机构跳转 |
...
- E 模块基础:工作台清单 / 拓展清单
- **K 模块全 P0**(2026-04-24 新增):K1-K5(GovOrg / GovContact 基础 CRUD + Visit 双轨关联 + 机构↔关键人关系 + 列表+详情);K6 评分作为 F13(V1.2+)
- 8.2 MVP 必需依赖 4 项接入(SSO / 行政区划 / 邮件告警 / 数据存储)
- ⚠️ V0.5 因 K 模块塞入,8 周可能拖到 9-10 周(见 9.4 风险因子)
...
| **F13** | 关键人活跃度评分(K6,2026-04-24 新增) | V1.2+ | 需要 V1.0 后档案积累足够数据 + 评分公式定义(基于 Visit 频次 + `last_engaged_at`) |

**V1.1 候选堆**(5-6 项):F2 / F6 / F7 / F8 / F12 / F10(可选)
**V1.2+ 候选堆**(7 项):F1 / F3 / F4 / F5 / F9 / F11 / **F13**
...
| **K 模块塞入 V0.5 范围**(2026-04-24 关键人档案融入) | V0.5 8 周可能拖到 9-10 周 | 仅做 K1-K5 基础 CRUD;K6 评分不在 V0.5;UI 复用现有 list / detail 模板;Visit 双轨设计避免一次性数据迁移 |
...
| **K 模块融入 V0.5 + F13 评分进 V1.2+**(2026-04-24) | 9.2 V0.5 加 K1-K5 / 9.3 加 F13 / 9.4 风险加一项 |
...
| **T15** | **候选分类标识** | **F 类** 功能性候选项(F1-F13 全部 9.3 分期,V1.1 / V1.2+ 两堆);**G 类** 累积议题/悬案(多在后续章节关账,全部已处置);**H 类** 权限/审计类悬案(全部第 7 章关账);**I 类** 信息架构/非功能类悬案(全部第 7 章关账);**K 类** 模块代号(K1-K6,2026-04-24 关键人档案融入新增) | 各章"兑现清单 / 新开悬案" | 本 PRD 内部标记约定,无外部参照;便于跨章追踪议题流转 |
| **T16** | **政府端档案(GovOrg + GovContact)**(2026-04-24 新增) | **GovOrg**:政府机构档案(K1)—— 名称 / 区划 / 层级 / 上级 / 职能标签;**GovContact**:关键人档案(K2)—— 姓名 / 所在机构 / 联系方式 / 备注;Visit 通过 `contact_id` 关联(K3 双轨,字符串字段作为兜底);**MVP 不做**字段级权限 / 分层 tier / 离职跟踪 / 评分(评分降为 F13) | 4.3.6 / 5.1.5 / K 模块 / T4 §5 | 2026-04-24 融入,**与 AI 版命名一致但去掉了字段级保护、分层、活跃度评分 P1**;沿用 7.1.1 ExportRecord 水印路线 |
...
| 2026-04-24 关键人档案融入(K 模块新增) | T16 政府端档案(GovOrg + GovContact) |
