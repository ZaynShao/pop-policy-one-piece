...
> **进度**:第 0、1 章已定稿(2026-04-22,跨章 review 后修订:删除"决策层"角色 + 多处级联调整),即将推进第 2 章。
...
| 项 | 内容 |
|---|---|
| **背景** | 公司战略转型新建的**属地一线**。驻点各省市,长期在当地;面孔多样:有从中台 GA 转岗、有外部新招、有跨业务线借调 |
| **日常** | (a) 拜访当地政府部门 / 园区 / 协会(高频) (b) 跟进政策动态、对接合作机会(高频) (c) 在系统内**录入拜访 / 上报情报 / 维护图钉** (d) 消化中台供应的工具包,转化为本地动作 (e) **识别监管风险、处理监管事件** ⚠️ 信息源 = 中台 GA 分发;源头数据库与分析能力 = **外部系统**(详见 0.7 #11、#12) |
| **核心诉求** | 一线在偏远地点也能轻量录入(响应式 H5);需要中台快速响应当地政策疑问;**不被中台用"任务流转"打扰**(呼应 0.4 ②) |
| **进系统第一眼想看** | 自己负责的省/市的本地态势(从**属地行动大盘**进入)+ 自己手头未完成的拜访/图钉 + 中台新发布的工具包 + **本省监管风险提醒** |
| **关系** | **接受负责人决策、自主执行**(PMO 不直接派单,只通过图钉/留言提供节奏参照);消费中台 GA 的产出(包括监管风险信息);数据为负责人提供"看清全国"的一手来源(再由负责人线下汇报决策层) |
| **粗粒度权限** | 自己负责的省市:全权读写(拜访、情报、图钉、留言);其他省市:只读;**不能编辑中台政策档案**;**不能编辑别人的拜访** |
...
| 属地 GA | 拜访 / 上报 / 消化工具 / 监管处置 | 当地政府 / 公司 | 自省读写 + 他省只读 |
...
### 待入第 4 章 · 核心数据模型
- **G3 同省多人**:一个省可能驻多个属地 GA,他们能否互编对方拜访?"省"还是"人"是数据主键?
- **G4 监管风险粒度**:监管事件挂"省/市/区/属地 GA 负责区域"哪一级?中台分发时如何路由到对应一线?
