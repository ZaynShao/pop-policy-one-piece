...
| **C** | 政策方向大盘(政策主题、涂层、勾选、覆盖清单) | 场景 1、3、4 | 待推进 |
| **D** | 工具池(成品文档 / 调用接口 / 生命周期) | 场景 1、2 | 待推进 |
| **E** | 综合看板(三轨进展、掉队预警、关键事件流) | 场景 4 | 待推进 |
| **F** | 个人工作台(消费明细、产出列表、当日动作留痕) | 场景 1、2、3 | 待推进 |
| **G** | 账号与权限(角色 CRUD、账号管理、数据归属转移) | 场景 5 | 待推进 |
| **H** | 审计与系统维护(审计日志、行政区划版本升级) | 场景 5 | 待推进 |
| **I** | 导出(截图 / PDF,含敏感字段脱敏) | 场景 4 | 待推进 |
| **J** | 外部依赖描述(政策分析系统 · 假 / 真) | 场景 2、3 | 仅描述性,不打优先级 |
