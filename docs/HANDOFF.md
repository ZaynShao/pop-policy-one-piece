...
### 7.3 下一步 = 第 7 章 · 非功能需求 OR 先做 AI 版对比矩阵

**两条可选路径**:

**路径 A · 继续主线推第 7 章**(非功能需求)
- 需要覆盖的遗留清单(备忘箱已累积):
  - G12 决策层服务路径的安全(水印 / 脱敏 / 留痕)
  - H1 AuditLog 跨角色读开放度
  - H2 ExportRecord 跨角色可见性
  - H3 Pin 中止发起人确认流程化
  - I1 热力 `mode=self` 分工交叉 —— 也可能放 F 类候选
  - 色彩公式的实测回调(来自 6.5)
  - 常规非功能项:性能 / 可用性 / 兼容性 / 监测告警 / 合规 / 灾备等(按业务体量裁剪)
- 依赖:第 1-6 章已足够
- 可选偷懒版或正式版

**路径 B · 开始做 AI 版对比矩阵**(`docs/PRD-comparison.md`)
- 按 `dual-prd-protocol.md` §4 的 8 维度(D1 覆盖范围 / D2 角色解读 / D3 优先级 / D4 字段命名 / D5 状态机 / D6 边界 / D7 非功能 / D8 盲点)逐章对比
- 用户版已到第 6 章,对比素材足够
- 对比产出可能反过来影响第 7-10 章

**建议**:主线推第 7 章之前,主对话应该先读一遍 `docs/PRD-ai-led.md` 建立印象(避免后面边读边写用户版第 7-10 章);用户按 §7 强约束短期内不看。若决定做对比矩阵可放在第 7 章之后或并行。

备忘箱当前未兑现:
- **G 类**:G4(监管风险粒度)/ G9(周观测公式)/ G12(水印脱敏)/ G13(种子数据)—— 共 4 项(G28 已在 6.6 处理)
- **H 类**:H1 AuditLog 跨角色读 / H2 ExportRecord 跨角色见 / H3 Pin 中止流程化 —— 共 3 项,都留第 7 章
- **I 类**(第 6 章新增):I1 热力 `mode=self` 分工交叉 —— 1 项,F 类候选
- **F 类**:F1-F7 候选功能
