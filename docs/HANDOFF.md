...
**第 5 章职责**:把各角色(local_ga / pmo / lead / central_ga / sys_admin)对各实体(Pin / PlanPoint / Visit / Tool / PolicyTheme / User / 等)的**CRUD + 状态变更 + 特殊操作**的权限落成一张矩阵表。

这是把第 1 章的"粗粒度权限"文字 + 第 4 章的实体 schema 落到**可执行的权限矩阵**。工作量相对可控(有规律),可以偷懒版或正式版推进。

备忘箱当前未兑现:
- **G 类**:G4(监管风险粒度)、G9(周观测公式)、G12(水印脱敏)、G13(种子数据)、G28(多涂层叠加)—— 共 5 项
- **F 类**:F1-F7 候选功能

**建议**:第 5 章可以继续偷懒版风格(默认按角色-实体矩阵机械生成),或正式做(逐格拍板)。MVP 阶段**偷懒版已足够**。
