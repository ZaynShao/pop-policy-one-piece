...
- ✅ `docs/PRD-user-led.md` **第 2 章 · 场景 1-3 已写**(2026-04-23):场景 1 属地 GA 一天开工 / 场景 2 中台 GA 维护工具 / 场景 3 中台 GA 维护政策主题

### 7.3 下一步 = 用户主导版第 2 章剩余场景

已覆盖:属地 GA 一线动作(场景 1)+ 中台 GA 两大供应动作(场景 2/3)。**待覆盖**:PMO、GA 负责人、系统管理员的代表场景。候选:
- PMO · 周观测/图钉协作(会触发 G9)
- 负责人 · 周度全局 review + 给决策层导出材料(触发 G12)
- 系统管理员 · MVP 阶段的日常管理动作

场景 2/3 也暴露了大量新概念(G18-G21、F6、涂层渲染规则、参数模板),在第 3-4 章要大量兑现。

第 2 章推进节奏:每完成一个场景,回看是否暴露前面章节的漏洞(场景 1 触发 1.3 修订;场景 2 触发 1.4 重写、术语表草案)。完稿后回填 2.1 场景地图。
