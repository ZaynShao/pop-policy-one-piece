...
### 7.3 下一步 = 第 4 章 · 核心数据模型

**第 4 章职责**:把 G18-G28 + B6/B9 蓝点/图钉状态机 + C4 涂层渲染 + 图钉/蓝点/拜访/工具/政策主题/覆盖清单/用户/角色/审计日志等实体 **落成 schema**(字段、类型、必填、关系、状态机)。

这是真正给开发团队看的章节,工作量最大。

备忘箱当前累积(第 4 章需兑现的主要是 G 类):
- **G 类**:G4、G9、G12、G13、G15-G28(共 15 项;G3、G14 已关闭)
- **F 类**:F1-F7(候选功能,本章不兑现)

**强烈建议**:第 4 章开始前,可考虑**暂停休整一下**,因为第 2-3 章已积累大量决策,需要沉淀;或者让 AI 版 sub-agent 先跑一轮(第 0-2 章完成,双轨时序允许启动 AI 版),用 AI 版结果反向 check 第 3 章的功能清单完整度。
