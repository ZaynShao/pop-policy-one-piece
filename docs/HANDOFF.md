...
**用户版 PRD**:**11/11 章完结 = 100% 🏁**
- 0-10 章全部定稿 —— 产品愿景 / 角色 / 场景 / 功能模块 / 数据模型 / 权限矩阵 / 信息架构 / 非功能 / 外部依赖 / 里程碑与分期 / **术语表**

**AI 版 PRD**:`docs/PRD-ai-led.md` 已入库(768 行),用户未看,主对话已读
**AI 版差异速览**:`docs/PRD-comparison-notes.md`(365 行,非正式笔记,D1-D8 维度)
**正式对比矩阵 `docs/PRD-comparison.md`**:**素材就绪,可即刻启动**(T4 阶段,3-4 小时工作量,双人协作)

**分支状态**:`claude/sad-kalam-a20dee`,ahead of origin/main by 7 commits(第 5 / 6 / 7 / 8 章 + HANDOFF 重启 + 第 9 章 + 第 10 章)

**备忘箱未兑现**:
- G / H / I 类:**全部已处置**
- F 类:F1-F12 全部已进池(第 9 章 9.3)
- 词条类:全部已兑现(第 10 章)
- **备忘箱全部关闭** 🏁
...
**Step 2 · 读 `docs/PRD-user-led.md` 末尾**:
- 第 10 章 10.末(🏁 用户版 PRD 正文完结提示)
- 备忘箱整体瞄一眼,确认已全部关闭
**Step 3 · 读 `docs/PRD-comparison-notes.md` 完整版** —— T4 对比矩阵的素材底;**绝对不要缩减这份笔记**(2026-04-23 夜用户明确强调:对比是完整的重要工作部分,不可压缩)
**Step 4 · 读 `docs/dual-prd-protocol.md` §4 + §8** —— T4 阶段按 8 维度产出正式对比矩阵,+ §8 元学习 5 题最终交付
**Step 5 · 按需读 `docs/PRD-ai-led.md`** —— 现在 AI 版可以正式对比,不再是"用户短期不看"阶段

### 7.5 下次 session 的入场话术(Claude 的开场)

读完后,不要立刻动手,先一句话复述:**"用户版 PRD 11 章全部完结 🏁,备忘箱全部关闭。下一步进 T4 正式对比矩阵 `docs/PRD-comparison.md`(3-4 小时,双人协作,按 8 维度逐章 × 差异点 × 整合建议)。分支 ahead of main by 7 commits。"**

然后直接问:**"现在就起 T4 对比矩阵?"** —— 用户 2026-04-23 夜已明确"第 10 章结束后就开始做 AI 版对比",默认进 T4,无需再给额外选项。

### 7.6 T4 对比矩阵启动清单

**就绪素材**
- 用户版 0-10 章已定稿(`docs/PRD-user-led.md`)
- AI 版 0-10 章已入库(`docs/PRD-ai-led.md`,768 行)
- 差异速览笔记 D1-D8(`docs/PRD-comparison-notes.md`,365 行)
- 元学习 5 题候选答案预填(`docs/PRD-comparison-notes.md` §9)

**T4 产出形态建议**
- 文件:`docs/PRD-comparison.md`(新建)
- 结构:
  1. 摘要 + 核心观察(从 notes §0 提取)
  2. 8 维度对比矩阵(D1 覆盖范围 / D2 角色 / D3 优先级 / D4 字段命名 / D5 状态机 / D6 边界 / D7 非功能 / D8 盲点互补)
  3. 逐章差异点表(0-10 章,每章列"共识 / 分歧 / 合并建议")
  4. 整合建议清单(哪些采用用户版 / 哪些借鉴 AI 版 / 哪些保留双轨)
  5. 元学习 5 题最终答案(用户填写)
- 工作量:3-4 小时,双人协作
- 完成后:POP 项目 PRD 阶段正式收官,进入技术架构文档阶段
...
| 7 commits | 3402260 / 3230662 / dbe482f / 45eb339 / 8db0577 / 第 9 章 / (本次第 10 章) | ahead of main |
...
## 9. 给新会话 Claude 的开场建议(2026-04-23 夜第 10 章完结后更新)

读完本文档 + `docs/PRD-user-led.md` 末尾 + `docs/PRD-comparison-notes.md` 后,**不要立刻动手干活**。先做两件事:

1. **一句话确认理解**:"用户版 PRD 11 章全部完结 🏁,备忘箱全部关闭。下一步进 T4 正式对比矩阵 `docs/PRD-comparison.md`(3-4 小时,双人协作)。分支 ahead of main by 7 commits。"

2. **直接问是否起 T4**:用户 2026-04-23 夜已明确"第 10 章结束后就开始做 AI 版对比",默认路径就是 T4,不再给额外选项。等用户说"开始 / 继续"即启动。

**不要做的事**:
- 不要缩减 `docs/PRD-comparison-notes.md`(用户明确强调不可压缩)
- 不要擅自进入 T4 之前的准备阶段(先按 §7.6 清单 + dual-prd-protocol §4/§8 对齐产出形态,再动手)
