...
### 7.3 下一步 = 第 8 章 · 外部依赖与未决项 OR 对比矩阵

**两条可选路径**:

**路径 A · 继续主线推第 8 章**(外部依赖与未决项)
- 需要覆盖:
  - 公司 SSO 对接(协议 / 开通流程 / 责任方)
  - 外部政策分析系统(G20):MVP 本地假系统,F6 真接入细节
  - 政策知识库接口(0 章提及)
  - G13 系统管理员种子数据导入(行政区划 / 初始用户 / 种子政策主题 / 责任方)
  - 监管信息源(F2 / G4,远期)
  - 其他 ToolBinding `endpoint_url` 指向的外部系统
- 依赖:第 1-7 章已足够
- 多为"登记外部依赖清单 + 责任方 / 时间线",可偷懒版

**路径 B · 开始做 AI 版对比矩阵**(`docs/PRD-comparison.md`)
- 按 `dual-prd-protocol.md` §4 的 8 维度逐章对比
- 用户版已到第 7 章(**7/11 章完成**,覆盖度够)
- 对比产出可能反过来影响第 8-10 章

**建议**:推第 8 章前,主对话应该先读一遍 `docs/PRD-ai-led.md` 建立印象(最晚节点,再拖下去 8-10 章写完就全失去纯度了);之后 A、B 的顺序由用户决定。

备忘箱当前未兑现:
- **G 类**:G4(监管风险粒度)/ G9(周观测公式)/ G13(种子数据)—— 共 3 项(G28 / G12 已处理)
- **H 类**:全部已处理(H1-H3 在第 7 章)
- **I 类**:I1 已处理(第 7 章);无新增
- **F 类**:F1-F7 候选功能 + **F8-F12**(第 7 章新增)= 共 12 项
