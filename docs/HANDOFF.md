...
### 7.3 下一步 = 第 6 章 · 信息架构与关键流程 OR 先做 AI 版对比矩阵

**两条可选路径**:

**路径 A · 继续主线推第 6 章**(信息架构与关键流程)
- 覆盖:导航结构 / 关键业务流程(属地 GA 开工 / 中台维护 / 负责人 review / 系统管理员日常)/ 页面地图 / 涂层渲染规则完整版(G28 相关)
- 依赖:第 1-5 章已足够
- 备忘箱 G28(多涂层叠加)在这章处理

**路径 B · 先做 AI 版对比矩阵**(`docs/PRD-comparison.md`)
- 按 `dual-prd-protocol.md` §4 的 8 维度(D1 覆盖范围 / D2 角色解读 / D3 优先级 / D4 字段命名 / D5 状态机 / D6 边界 / D7 非功能 / D8 盲点)逐章对比
- 用户版已到第 5 章,覆盖度够,可以开始对比
- 对比产出反过来可能修订备忘箱、甚至重判某些偷懒版的章节

**建议**:**路径 A + B 可并行**。主对话先读一遍 AI 版建立印象,但对比矩阵留到第 6-7 章也大致出了后再正式写(对比维度更完整);主线继续推第 6 章不停。

备忘箱当前未兑现:
- **G 类**:G4(监管风险粒度)/ G9(周观测公式)/ G12(水印脱敏)/ G13(种子数据)/ G28(多涂层叠加)—— 共 5 项
- **H 类**(第 5 章新增):H1 AuditLog 跨角色读 / H2 ExportRecord 跨角色见 / H3 Pin 中止流程化 —— 共 3 项,都留第 7 章
- **F 类**:F1-F7 候选功能
